...
test('Communication routes authenticate callers and fail safely without real providers', { timeout: 60_000 }, async (t) => {
  ...
  assert.equal((await request(`${leaguePath}/deposits`)).status, 401);
  assert.equal((await request(`${leaguePath}/admin/deposits`, { headers: playerHeaders })).status, 401);
  assert.equal((await post('/api/leagues/another-league/deposits', { amount: 10 }, playerHeaders)).status, 403);
  await request(`${leaguePath}/cashapp-pool`, { method: 'PATCH', headers, body: JSON.stringify({ url: 'https://cash.app/$test' }) });
  const depositResponse = await post(`${leaguePath}/deposits`, { amount: 20, playerId: 'player-chris', requestId: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa' }, playerHeaders);
  assert.equal(depositResponse.status, 201);
  const { deposit } = await depositResponse.json();
  assert.equal(deposit.playerId, 'player-route-test');
  assert.equal((await post(`${leaguePath}/admin/deposits/${deposit.id}`, { action: 'confirm' }, playerHeaders)).status, 401);
  assert.equal((await post(`${leaguePath}/admin/deposits/${deposit.id}`, { action: 'confirm' }, headers)).status, 200);
  assert.equal((await post(`${leaguePath}/admin/deposits/${deposit.id}`, { action: 'confirm' }, headers)).status, 200);
  const depositPayments = await (await request(`${leaguePath}/payment-history`, { headers: playerHeaders })).json();
  assert.equal(depositPayments.summary.creditBalance, 20);
  const depositNotes = await (await request(`${leaguePath}/notifications`, { headers: playerHeaders })).json();
  assert.equal(depositNotes.notifications.filter(n => n.kind === 'deposit').length, 1);
