...
export function createJackTtsProvider(env = process.env, fetchImpl = fetch) {
  const kind = String(env.JACK_TTS_PROVIDER ?? 'browser').trim().toLowerCase();
  ...
  const apiKey = String(env.JACK_TTS_API_KEY ?? '').trim();
  const voiceId = String(env.JACK_TTS_VOICE_ID ?? '').trim();
  const configured = Boolean(apiKey && voiceId);
  return {
    kind,
    configured,
    profile: 'deep-warm-original-designed-voice',
    async synthesize({ text, speed } = {}) {
      ...
      const response = await fetchImpl(`${ELEVENLABS_BASE}/${encodeURIComponent(voiceId)}?output_format=mp3_44100_128`, {
        ...
        body: JSON.stringify({
          ...
          // Preserve the selected voice's stored settings by default.
          ...(speed == null ? {} : { voice_settings: { speed: Math.min(1.2, Math.max(0.8, Number(speed) || 0.94)) } }),
        }),
      ...
      if (!contentType.startsWith('audio/')) throw new Error('Voice provider returned non-audio content.');
    ...
    async diagnose({ testAudio = false } = {}) {
      ...
      // Bound diagnostics to one lookup window, even if an account endpoint is
      // slow or unavailable. One failed lookup must not hide the other result.
      await Promise.all([
        (async () => {
          try {
          ...
          } catch (error) { out.voiceLookup = { error: error.message }; }
        })(),
        (async () => {
          try {
          ...
          } catch (error) { out.subscription = { error: error.message }; }
        })(),
      ]);
      if (testAudio) try {
