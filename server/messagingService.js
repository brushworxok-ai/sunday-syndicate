...
export class TelnyxSmsProvider {
  constructor({ apiKey, fromNumber, fetchImpl = fetch }) {
  ...
  async send({ player, text }) {
    ...
    if (!response.ok || result.errors?.length || ['sending_failed', 'delivery_failed'].includes(result.data?.to?.[0]?.status)) {
    ...
    if (!result.data?.id) {
      const error = new Error('Telnyx returned no trackable message ID. Delivery is unknown; do not automatically resend.');
      error.retryable = false;
      throw error;
    }
    return { status: result.data?.to?.[0]?.status ?? 'queued', id: result.data.id };
  }

  /* Telnyx group MMS uses a dedicated endpoint and at most eight recipients. */
  async sendGroup({ players, text }) {
    ...
    const result = await response.json().catch(() => ({}));
    if (!response.ok || result.errors?.length || !result.data?.id) throw new Error(result.errors?.[0]?.detail ?? `Telnyx group MMS was not accepted (HTTP ${response.status}).`);
    return { status: 'queued', id: result.data.id };
  }

  /* Look up what happened to a message after Telnyx accepted it. */
  async messageStatus(id) {
    const res = await this.fetch(`https://api.telnyx.com/v2/messages/${encodeURIComponent(id)}`, { headers: { Authorization: `Bearer ${this.apiKey}` }, signal: AbortSignal.timeout(8_000) });
  ...
  async diagnose() {
    ...
    const numRes = await this.fetch(`https://api.telnyx.com/v2/phone_numbers?filter[phone_number]=${encodeURIComponent(this.fromNumber)}`, { headers, signal: AbortSignal.timeout(8_000) });
    const numJson = await numRes.json().catch(() => ({}));
    out.apiKeyValid = numRes.ok;
...
export class TextBeltSmsProvider {
  ...
  async send({ player, text }) {
    ...
    return { status: 'queued', id: String(result.textId ?? randomUUID()) };
  }
}
...
export async function sendTextBeltRaw({ phone, text, apiKey }) {
  ...
  const result = await response.json();
  if (!response.ok || !result.success) throw new Error(result.error || 'TextBelt delivery failed');
  ...
  for (const summary of league.players) {
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      ...
      try {
        ...
        lastError = error;
        if (error.retryable === false || ['TimeoutError', 'AbortError', 'TypeError'].includes(error.name)) break;
      }
    }
    if (delivered) deliveries.push(delivered);
    else deliveries.push({ playerId: player.id, channel: 'sms', status: 'failed', providerAttempted: true, provider: provider.name, errorCode: String(lastError?.code ?? 'provider_error'), error: lastError?.message ?? 'Provider failed', attemptCount, attemptedAt: new Date().toISOString(), retryStatus: attemptCount > 1 ? 'paused_after_one_retry' : 'not_retried', fallback: { channel: 'in_app', status: 'delivered', deliveredAt: new Date().toISOString() } });
  }
...
export async function sendJackBroadcast({ store, leagueId, provider, messages, actor = 'jack', kind = 'jack_broadcast' }) {
  ...
  for (const msg of messages) {
    ...
    if (delivered) deliveries.push(delivered);
    else deliveries.push({ playerId: msg.playerId, channel: 'sms', status: 'failed', providerAttempted: true, provider: provider.name, errorCode: String(lastError?.code ?? 'provider_error'), error: lastError?.message ?? 'Provider failed', attemptCount, attemptedAt: new Date().toISOString() });
  }
...
export async function applyDeliveryStatus({ store, providerMessageId, status, errorCode = null, actor = 'twilio_webhook' }) {
  if (!providerMessageId || !['queued', 'sending', 'sent', 'delivered', 'failed', 'undelivered', 'delivery_unconfirmed'].includes(status)) return null;
  ...
  if (delivery.status === status || (['delivered', 'failed'].includes(delivery.status) && ['queued', 'sending', 'sent', 'delivery_unconfirmed'].includes(status))) return delivery;
  ...
  await store.writeAudit(found.leagueId, 'delivery.status_updated', `Provider delivery ${providerMessageId} changed to ${status}`, actor, { providerMessageId, status, errorCode });
