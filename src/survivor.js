...
import { TEAMS, getGames, DEADLINE_LABEL } from './data.js';
...
export function validateSurvivorPick({ playerId, week, team, survivorPicks = [], results = {}, players = [], isWeekLocked }) {
  ...
  if (typeof isWeekLocked === 'function' && isWeekLocked(week)) return { ok: false, error: `Week ${week} is locked — picks are due ${DEADLINE_LABEL} before the first kickoff.` };
